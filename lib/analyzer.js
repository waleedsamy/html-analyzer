...
function extractHypermediaLinks($, check_https_support) {
    return when.promise(function(resolve, reject) {
        ...
        // check if link support https by hitting the https version of the url with head request
                    ...
                    request.head({
                        ...
                        if (error || response.statusCode !== 200) {
                            return resolve({
                                link: link,
                                https: 'not supported',
                                reason: (response && response.statusCode) ? response.statusCode : 'NO RESPONSE'
                            });
                        } else {
                            return resolve({
                                link: link,
